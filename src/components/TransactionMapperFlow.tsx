...
import { Check, X, ChevronLeft, ChevronRight, Sparkles, Plus, ArrowDownUp, Trash2 } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { useAuth } from '../contexts/AuthContext';
import { Transaction, Category } from '../lib/types';

interface TransactionMapperFlowProps {
  batchId?: string;
  onClose: () => void;
}

export default function TransactionMapperFlow({ batchId, onClose }: TransactionMapperFlowProps) {
  const { user } = useAuth();
  ...
  const handleSaveAndNext = async () => {
    ...
    try {
      ...
      if (currentIndex >= newTransactions.length && currentIndex > 0) {
  ...
  const handleConfirmAllWithAI = async () => {
    ...
    try {
      ...
      const remainingTransactions = transactions.filter(t => !validUpdates.find(u => u.id === t.id));
      ...
      if (remainingTransactions.length === 0) {
        ...
        setCurrentIndex(0);
        alert(`Successfully approved ${validUpdates.length} transactions. ${remainingTransactions.length} transactions remain for manual mapping.`);
      }
    } catch (error) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <span
            className={`px-4 py-2 rounded-full text-sm font-semibold ${
              currentTransaction.type === 'credit' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
            }`}
          >
          ...
          <span className="text-2xl font-bold text-gray-900">
            ₹{currentTransaction.amount.toLocaleString('en-IN', { maximumFractionDigits: 2 })}
          </span>
        </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg font-medium ${
                      newCategory.type === 'expense' ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700'
                    }`}
                  >
                    Expense
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg font-medium ${
                      newCategory.type === 'income' ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700'
                    }`}
                  >
                    Income
                  </button>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 py-4 rounded-xl font-semibold text-red-700 bg-red-50 hover:bg-red-100 transition-colors"
        >
